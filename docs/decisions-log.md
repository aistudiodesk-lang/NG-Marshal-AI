...
## 1 Aug 2026 — Drivers tab: driver-wise incentive dashboard

New **Drivers** tab — the driver-wise incentive dashboard the user asked for, built on the same trip-log data (manual / uploaded / transport report). Per driver: trips, TEU, TEU-by-movement (import/export/scanning/check-pkg), and **₹ incentive** computed from the rate card (per-TEU rates + milestone bonus on any day clearing the target). Every column is click-to-sort, a date filter narrows it to any day or the whole period, and "⬇ Incentive report" exports the detailed driver-wise CSV. `logIncentive()` / `logTeuByMovement()` helpers in types.ts. Verified on the July transport report: 355 units · 20,351 trips · 33,499 TEU · ₹27,80,520 total, sorted by earnings. When the ITV master is loaded (truck reg → call sign → driver), the per-truck rows collapse to real driver names. Trip Log and Drivers are two lenses on one dataset; both date-filterable, both sortable.
