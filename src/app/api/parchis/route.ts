...
interface Row { driver_id: string; driver_name: string; storage_path: string; captured_at: string }
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await sb
    .from("parchi_photos")
    .select("driver_id,driver_name,storage_path,captured_at")
  ...
  const groups = new Map<string, { driverId: string; driverName: string; photos: { url: string | null; capturedAt: string }[] }>();
  for (const r of rows) {
    if (!groups.has(r.driver_id)) groups.set(r.driver_id, { driverId: r.driver_id, driverName: r.driver_name, photos: [] });
    groups.get(r.driver_id)!.photos.push({ url: signed.get(r.storage_path) ?? null, capturedAt: r.captured_at });
  }
