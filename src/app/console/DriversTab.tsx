...
import { logTrips, logTeu, logTeuByMovement, logIncentive } from "@/lib/types";
...
type SortKey = "driver" | "trips" | "teu" | "imp" | "exp" | "scan" | "incentive";
...
export default function DriversTab() {
  ...
  const rows = useMemo(() => {
    ...
    logs.forEach((l) => {
      ...
      if (!m.has(key)) m.set(key, { driver: l.driverName || `(ITV ${l.itv})`, vendor: l.vendor ?? "—", itvs: new Set(), trips: 0, teu: 0, imp: 0, exp: 0, scan: 0, cp: 0, incentive: 0, days: new Set() });
      const g = m.get(key)!;
    ...
    const arr = [...m.values()];
    arr.sort((a, b) => {
      const v = (r: Row) => (sortKey === "driver" ? r.driver : r[sortKey]);
      const av = v(a), bv = v(b);
  ...
  const exportCsv = () => {
    const head = ["Driver", "Vendor", "ITV(s)", "Days", "Trips", "Import TEU", "Export TEU", "Scanning TEU", "Check pkg", "Total TEU", "Incentive INR"];
    const body = rows.map((r) => [r.driver, r.vendor, [...r.itvs].join(" "), r.days.size, r.trips, r.imp, r.exp, r.scan, r.cp, r.teu, r.incentive]);
